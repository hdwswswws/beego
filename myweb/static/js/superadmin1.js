...
$.ajax({
    type: "POST",
    url: 'http://localhost:8081/api/finduser',
    dataType: "json",
    data: { page: 1, pagesize: 10, username: "", state: 0 },
    success: function (res) {
        ...
        for (var i = 0; i < res.data.length; i++) {
            if (res.data[i].active_state == 1) {
                $("#tbody1").append("<tr><td>" + (i + 1) + "</td><td>" + res.data[i].username + "</td><td>正常</td><td>" + rTime(res.data[i].created_at) + "</td><td><div class='btn-group'> <a onclick='editpwd(this)' name='" + res.data[i].username + "' class='btn btn-default'>修改密码</a><a onclick='editstate1(this)' name='" + res.data[i].username + "' class='btn btn-default'>禁用</a><a onclick='del(this)' name='" + res.data[i].username + "' class='btn btn-danger'>删除</a></div></td></tr>");
            }
            else {
                $("#tbody1").append("<tr><td>" + (i + 1) + "</td><td>" + res.data[i].username + "</td><td>冻结</td><td>" + rTime(res.data[i].created_at) + "</td><td><div class='btn-group'> <a onclick='editpwd(this)' name='" + res.data[i].username + "' class='btn btn-default'>修改密码</a><a onclick='editstate2(this)' name='" + res.data[i].username + "' class='btn btn-default'>激活</a><a onclick='del(this)' name='" + res.data[i].username + "' class='btn btn-danger'>删除</a></div></td></tr>");
            }
        }
...
function editpwd(val) {
    ...
    $.ajax({
        type: "POST",
        url: 'http://localhost:8081/api/edituserpassword',
        dataType: "json",
        data: { username: val.name, password: pwd },
        success: function (res) {
            alert(JSON.stringify(res.message));
            refreshpage();
        },
...
function editstate1(val) {
    ...
    $.ajax({
        type: "POST",
        url: 'http://localhost:8081/api/edituserstate',
        dataType: "json",
...
function del(val) {
    ...
    $.ajax({
        type: "POST",
        url: 'http://localhost:8081/api/deluser',
        dataType: "json",
...
function selectpage(val) {
    ...
    $.ajax({
        ...
        url: 'http://localhost:8081/api/finduser',
        dataType: "json",
        data: { page: val.name, pagesize: 10, username: username, state: state },
        success: function (res) {
            if (res.code == 1) {
                alert(JSON.stringify(res.message));
                return;
...
function refreshpage() {
    ...
    $.ajax({
        ...
        url: 'http://localhost:8081/api/finduser',
        dataType: "json",
        data: { page: page, pagesize: 10, username: username, state: state },
        success: function (res) {
            if (res.code == 1) {
                alert(JSON.stringify(res.message));
                return;
...
function search() {
    ...
    $.ajax({
        ...
        url: 'http://localhost:8081/api/finduser',
        dataType: "json",
        data: { page: 1, pagesize: 10, username: username, state: state },
        success: function (res) {
            if (res.code == 1) {
                alert(JSON.stringify(res.message));
                return;
