...
$.ajax({
    type: "POST",
    url: 'http://localhost:8081/api/findclubnotice',
    dataType: "json",
    ...
    success: function (res) {
        ...
        for (var i = 0; i < res.data.length; i++) {
            $("#tbody1").append("<tr><td>" + (i + 1) + "</td><td>" + res.data[i].notice + "</td><td>" + rTime(res.data[i].created_at) + "</td><td><div class='btn-group'><a onclick='editnotice(this)' name='" + res.data[i].id + "' class='btn btn-default'>修改通知</a> <a  onclick='delnotice(this)' name='" + res.data[i].id + "' class='btn btn-danger'>删除通知</a></div></td></tr>");
        }
...
var changenoticeid = ""
function editnotice(val) {
    changenoticeid = val.name
    $("#myModal4").modal("show")
}
function changenotice() {
    $.ajax({
        type: "POST",
        url: 'http://localhost:8081/api/editclubnotice',
        dataType: "json",
        data: { id: changenoticeid, notice: $("#notice2").val() },
        success: function (res) {
            alert(JSON.stringify(res.message));
            $("#notice2").val("")
            refreshpage();
...
function createnotice() {
    $.ajax({
        type: "POST",
        url: 'http://localhost:8081/api/createclubnotice',
        dataType: "json",
        data: { notice: $("#notice1").val()},
        success: function (res) {
            alert(JSON.stringify(res.message));
            $("#notice1").val("")
            refreshpage();
...
function delnotice(val) {
    if (confirm('确认删除通知 ？') == false) {
        return
    }
    $.ajax({
        type: "POST",
        url: 'http://localhost:8081/api/delclubnotice',
        dataType: "json",
        data: { id: val.name },
        success: function (res) {
            alert(JSON.stringify(res.message));
